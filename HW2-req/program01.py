...
def lastRing(a, b, k):
    ...
    if diff != 0:
        if diff > k:
            return (a, b).index(min(a, b))
        else:
            return (a, b).index(max(a, b))
...
def normalize(lista):
    ...
    for sublista in lista:
        sublist = []
        for char in sublista:
            if char != " " and char != "\t":
                sublist.append(ord(char))
        outerlist.append(sublist)
    return outerlist
    #return [[y for y in x if x != " " and x != "\t"] for x in lista]
...
def compare(a, b, k):
    # results = [0, 0]
    thing = list(map(lambda x, y: lastRing(x, y, k), a, b))
    count1 = thing.count(1)
    count0 = thing.count(0)
    # x, y = results
    if count0 > count1:
        return 0
    elif count0 < count1:
        ...
        if i > j:
            ...
            if "".join([chr(x) for x in a]) > "".join([chr(x) for x in b]):
                return 0
            else:
                return 1


def createCombinations(lista):
...
def ex(matches, k):
    ...
    length = len(matches)
    combinations = createCombinations(length)

    for item in range(length):
        results[item] = 0
    for x, y in combinations:
        scores = [0, 0]
        #for elem in list(map(lambda x, y: lastRing(x, y, k), matches[n], matches[m])):
            #scores[n if elem == 0 else m] += 1
        #results[player] += 1
    return list(sort_dict_by_value(results).keys())
