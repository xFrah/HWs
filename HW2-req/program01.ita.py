...
def lastRing(a, b):
    ...
    b = ord(b)
    diff = abs(a - b)
    maxI = (a, b).index(max(a, b))
    return diff, maxI


def normalize(lista):
    return [x for x in lista if x != " " and x != "\t"]


def compare(a, b, k):
    a = normalize(a)
    b = normalize(b)
    thing = list(map(lastRing, a, b))
    results = [0, 0]
    for diff, maxI in thing:
        if diff != 0:
            if diff > k:
                results[1 - maxI] += 1
            elif diff <= k:
...
def ex(matches, k):
    results = [0] * len(matches)
    setlist = set()
    for n, x in enumerate(matches):
        print(matches)
        print(matches[:n] + matches[(n + 1):])
        for y in matches[:n] + matches[(n + 1):]:
            m = matches.index(y)
            fset = frozenset((n, m))
            if fset not in setlist:
                end = compare(x, y, k)
                if end == 0:
                    results[n] += 1
                elif end == 1:
...
if __name__ == "__main__":
    print(ex(["ab      c",  "db    a", "e      Zo"], 10))
    print({"a", "c"} == frozenset(["c", "a"]))